...
class FancyPg extends Sprite {
	...
	public function new() {
		...
		var guiDcDescr = Xml.parse(GuiDrawcalls.DRAWCALLS_LAYOUT).firstElement();
		guiDcDescr.addChild(Xml.parse(PictureDrawcalls.DRAWCALLS_LAYOUT(pictureFile)).firstElement());
		var root:Entity = fuiBuilder.createDefaultRoot(guiDcDescr);
		createTextStyles();
